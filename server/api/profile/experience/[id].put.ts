export default defineEventHandler(async (event) => {
  ...
  const result = await db.query(
    `UPDATE profile_experience
     ...
     RETURNING *`,
    [data.company, data.role, data.startDate, data.endDate, data.description, data.isCurrent, data.location, data.employmentType, data.techStack, data.highlights, data.reasonForLeaving, id, user.id],
  )
