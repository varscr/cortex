export default defineEventHandler(async (event) => {
  ...
  const result = await db.query(
    `UPDATE knowledge_entries
     ...
     RETURNING *`,
    [data.title, data.content, data.category, data.tags, data.confidence, data.sourceConversationId, data.sourceConversationTitle, data.isReviewed ?? null, contentHash, id, user.id]
  )
